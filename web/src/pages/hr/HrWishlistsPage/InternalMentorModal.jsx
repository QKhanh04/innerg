...
export default function InternalMentorModal({ wishlist, onClose }) {
    ...
    const recommended = filteredSuggestions.filter((s) => s.isRecommended);
    const others = filteredSuggestions.filter((s) => !s.isRecommended);

    const renderTrainer = (trainer) => (
        <div
            key={trainer.trainerId}
            className={cn(
                'flex items-center justify-between p-4 bg-white rounded-xl border transition-all group shadow-sm',
                trainer.isRecommended
                    ? 'border-primary/30 hover:border-primary/50 hover:bg-slate-50/50'
                    ...
                    <div className="relative flex items-center justify-between">
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary/80">Gán Mentor</span>
                            </div>
                            <h2 className="text-xl font-bold tracking-tight">Chọn Mentor nội bộ</h2>
                            <p className="text-xs text-slate-300 font-medium mt-1">Chủ đề: <span className="text-white">{wishlist.skillName}</span></p>
                        </div>
                        <button
                            onClick={onClose}
