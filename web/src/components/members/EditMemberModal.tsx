        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md bg-white rounded-2xl shadow-xl overflow-hidden animate-slideIn">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    <div>
                        <label className="block text-xs font-bold text-slate-600 mb-1.5 uppercase tracking-wide">Nhân viên</label>
                        <div className="px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-sm font-medium text-slate-700">
                            {member.name} - {member.email}
                        </div>
                        <p className="text-[10px] text-slate-400 mt-1">Không thể đổi tên và email ở màn hình này.</p>
                    </div>

                    <div>
                        <label className="block text-xs font-bold text-slate-600 mb-1.5 uppercase tracking-wide">Chức vụ</label>
                        <input
                            type="text"
                    ...
                    <div className="pt-4 flex items-center justify-end gap-3">
                        ...
                        >
                            {isUpdating ? 'Đang lưu...' : <><Check className="w-4 h-4" /> Lưu thay đổi</>}
                        </button>
                    </div>
                </form>
            </div>
